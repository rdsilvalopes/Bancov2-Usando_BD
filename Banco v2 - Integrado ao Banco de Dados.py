...
class CadastroUsuario:
    ...
    conn = sqlite3.connect('C:/Banco_Brasil/banco_brasil.db')
    ...
    #     self.c.execute('CREATE TABLE IF NOT EXISTS conta(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, '
    #                    'valor_deposito REAL, data_deposito TEXT, valor_saque REAL, data_saque TEXT, '
    #                     'nome TEXT)')


    def __init__(self, nome, sobrenome, saldo, deposito, valor_deposito, data_deposito):
        self.nome = nome
    ...
    def set_deposito(self, valor, nome):
        self.data_deposito = datetime.datetime.today()
        self.valor_deposito = valor
        self.nome = nome

        self.c.execute("INSERT INTO conta (data_deposito, valor_deposito, nome) VALUES(?, ?, ?)",
                       (self.data_deposito.strftime('%D %H:%M:%S'),
                        self.valor_deposito,
                        self.nome))
    ...
    def valida_deposito():
        while True:
            ...
            # Tratamento para que não seja utilizado string
            try:
                dados.valor_deposito = float(input('>>> R$ '))
            except ValueError:
                print('-----------------------------')
            ...
            if dados.valor_deposito <= 0:
                ...
                print('>>> R${0:.2f}'.format(dados.set_deposito(dados.valor_deposito, dados.nome)))

                  # Deposito realizado com sucesso, a tentativa com FALHA que conta os ERROS é zerada
                dados.tentativa = 0
            break


dados = CadastroUsuario(nome='', sobrenome='', saldo=0, deposito=0, valor_deposito='', data_deposito='')
